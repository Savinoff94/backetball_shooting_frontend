...
function InputFieldStyled({inputVal, htmlFor, labelText, onChange, type = 'text', isError = false} : InputFieldProps) {
    ...
    return (
        <div className="flex flex-col content-evenly gap-1">
            ...
            <input data-is-error={isError} type={type} className="ml-4 mr-4 h-12 rounded-lg p-2 border data-[is-error=false]:border-purple-200 data-[is-error=true]:border-red-400"  value={inputVal} onChange={(event: React.FormEvent<HTMLInputElement>) => onChange(event)} name={htmlFor}  required/>
        </div>
    );
}
